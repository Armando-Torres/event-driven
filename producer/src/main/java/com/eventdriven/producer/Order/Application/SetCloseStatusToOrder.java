...
public class SetCloseStatusToOrder {
    private final String topicName = "customer-order";
    ...
    private void sendOrderCloseEvent(Order order) {
        ...
        try {
            this.messageProducer.sendMessage(this.topicName, order.getId().toString(), objectMapper.writeValueAsString(order));
